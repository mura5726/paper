---
title: "Summary of 'Secure Key Generation for Static Visual Watermarking by Machine learning in Intelligent Systems and Services'"
...
4. How can the efficiency be validated 
...
- Conventional cryptographic schemes are highly power consuming and time consuming, which are not suitable for these types of devices so that necessary information must be obtainable with less calculation cost and power consumption.
...
- Because author's proposed method does not affect the target content at all, one can apply new watermark seamlessly without altering the fingerprint using the proposed method.
